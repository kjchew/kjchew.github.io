...
export default {
  ...
  methods: {
    addMetadata(emailName, campaignTag) {
      ...
      axios
        .post(`${cloudpageUrl}?deName=poc_kj_metadata`, payload)
        .then((response) => {
    ...
    updateMetadata(index, emailName, campaignTag) {
      ...
      axios
        .put(`${cloudpageUrl}?deName=poc_kj_metadata`, payload)
        .then((response) => {
...
function sortCampaigns(campA, campB) {
  if (campA.campaignTag === campB.campaignTag)
    return campA.emailName > campB.emailName ? 1 : -1;
